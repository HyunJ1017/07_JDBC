...
public class UserServiceImpl implements UserService{
	...
	@Override
	public List<User> searchId(String searchId) throws Exception  {
		Connection conn = getConnection();
		// 데이터가공
		searchId = '%' + searchId + '%';	// %검색어% 형태로 가공
		List<User> userList = dao.searchId(conn, searchId);
		close(conn);
		return userList;
	}
	...
	public User selectUser(String userNo) throws Exception {
		...
		Connection conn = getConnection();
		int searchNo = Integer.parseInt(userNo);
		User user = dao.selectUser(conn, searchNo);
		close(conn);
